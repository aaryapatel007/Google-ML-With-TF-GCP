...
def dnn_model(img, mode, hparams):
  ...
  #TODO: Implement DNN model with three hiddenlayers
    ...
    X = tf.reshape(img,[-1,HEIGHT*WIDTH]) #flatten
    h1 = tf.layers.dense(X,128,activation = tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer())
    h2 = tf.layers.dense(h1,64,activation = tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer())
    h3 = tf.layers.dense(h2,32,activation = tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer())
