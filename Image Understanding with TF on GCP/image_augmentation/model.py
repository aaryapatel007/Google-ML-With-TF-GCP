...
def cnn_model(img, mode, hparams):
  ...
  a = tf.reshape(img,[-1,HEIGHT,WIDTH,NUM_CHANNELS])
  c1 = tf.layers.conv2d(a, filters=nfil1,
...
def read_and_preprocess(image_bytes, label=None, augment=False):
    ...
    image = tf.image.decode_image(image_bytes,channels = 3)#TODO: decode contents into JPEG
    image = tf.image.convert_image_dtype(image,tf.float32)#TODO: convert JPEG tensor to floats between 0 and 1
    ...
    if augment:
       ...
       image = tf.image.random_brightness(image,63.0/255.0)
