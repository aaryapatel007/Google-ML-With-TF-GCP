...
def dnn_model(img, mode, hparams):
  ...
  X = tf.reshape(img,[-1,HEIGHT*WIDTH]) #flatten
  h1 = tf.layers.dense(X,256,activation = tf.nn.leaky_relu,initializer = tf.contrib.layers.xavier_initializer())
  h2 = tf.layers.dense(h1,128,activation = tf.nn.leaky_relu,initializer = tf.contrib.layers.xavier_initializer())
  h3 = tf.layers.dense(h2,32,activation = tf.nn.leaky_relu,initializer = tf.contrib.layers.xavier_initializer())
