...
def cnn_model(img, mode, hparams):
  ...
  c1 = tf.layers.conv2d(img, filters=nfil1,
                          kernel_size=ksize1, strides=(1,1), # ?x28x28x10
                          padding='same')
  c1a = tf.nn.relu(c1)
  p1 = tf.layers.max_pooling2d(c1a,pool_size=2, strides=2) # ?x14x14x10
  c2 = tf.layers.conv2d(p1, filters=nfil2,
                          kernel_size=ksize2, strides=(1,1), 
                          padding='same')
  c2a = tf.nn.leaky_relu(c2)
  p2 = tf.layers.max_pooling2d(c2a,pool_size=2, strides=2) # ?x7x7x20
  
  #outlen = p2.shape[1]*p2.shape[2]*p2.shape[3] #980
  #p2flat = tf.reshape(p2, [-1, outlen]) # flattened
  p2flat = tf.layers.flatten(p2)
